...
async function analyzeVideo(
  ...
  try {
    ...
    throw new Error('Gemini returned an unexpected response format. Please retry.');
  }
}
...
export async function processTaskVideo(
  ...
  if (!GEMINI_API_KEY) {
    throw new Error('Missing Gemini API key. Set EXPO_PUBLIC_GEMINI_API_KEY in .env and restart the app.');
  }
...
export function taskConcernScore(result: TaskAnalysisResult): number {
  ...
  // Eye contact (0–3)
  score += { none: 3, rare: 2, occasional: 1, frequent: 0, consistent: 0 }[result.eye_contact.frequency] ?? 1;
  ...
  if (result.name_response.applicable) {
    score += { yes: 0, partial: 1, no: 3, not_applicable: 0 }[result.name_response.responded] ?? 1;
  }

  // Shared attention (0–3)
  score += { none: 3, minimal: 2, emerging: 1, clear: 0 }[result.shared_attention.quality] ?? 1;
