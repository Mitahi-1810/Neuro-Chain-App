...
const syncTables = async () => {
  ...
  if (unsyncedSpecialists.length > 0) {
    console.log(`Found ${unsyncedSpecialists.length} unsynced specialists. Pushing to Supabase...`);
    ...
    for (const specialist of unsyncedSpecialists as any[]) {
      await db.runAsync('UPDATE specialists SET sync_status = 1 WHERE id = ?', [specialist.id]);
    }
  ...
  if (unsyncedUsers.length > 0) {
    console.log(`Found ${unsyncedUsers.length} unsynced users. Pushing to Supabase...`);
    ...
    for (const user of unsyncedUsers as any[]) {
      await db.runAsync('UPDATE users SET sync_status = 1 WHERE id = ?', [user.id]);
    }
  ...
  if (unsyncedAppointments.length > 0) {
    console.log(`Found ${unsyncedAppointments.length} unsynced appointments. Pushing to Supabase...`);
    ...
    for (const appointment of unsyncedAppointments as any[]) {
      await db.runAsync('UPDATE appointments SET sync_status = 1 WHERE id = ?', [appointment.id]);
    }
